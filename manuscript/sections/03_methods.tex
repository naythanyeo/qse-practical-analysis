...
In this study, six ansatz families were used, namely: UCCSD, UCCSDSinglet, UCCGSD, and \(k\)-UpCCGSDSinglet with \(k=1,2,\) and \(3\). The Ansatz were characterised by the excitation generators included in the cluster operator $\hat{T}$, summarised in the following table.
...
wherby $i$ and $j$ to denote active spatial orbitals.
...
A projected observable matrix were constructed for the overlap, Hamiltonian and spin-squared matrices. Observable matrix elements were defined as follows: 
...
Due to the non-orthogonality of the QSE basis states, solving for the energy eigenvalues and wavefunctions involves solving a generalised eigenvalue problem $Hc = ESc$. This was done by first diagonalising the Hermitian overlap matrix $S$. 

\DiagonaliseSEquation
where the columns of $U$ are the eigenvectors of $S$ and $\Lambda$ is a diagonal matrix containing the eigenvalues of $S$. Then, we define the whitening factor
