...
\begin{figure}[!ht]
    ...
    \caption{Double-excitation weight of the exact CASCI low-lying singlet and triplet states across active spaces. Solid lines show the median double-excitation weight for the $S_0$, $S_1$, $T_1$, and $T_2$ states across the molecular benchmark; shaded regions show the corresponding distribution ranges.}
...
\begin{figure}[!ht]
    ...
    \caption{Molecule-level index-matched errors for the low-lying $S_0$, $S_1$, $T_1$, and $T_2$ roots in the UCCSD $6e6o$ active space, comparing the maximum retained QSE dimension with the overlap-stable dimension.}
...
\begin{figure}[!ht]
    ...
    \caption{Mean optimal overlap-eigenvalue threshold across active spaces and shot counts. Each value is selected from the threshold ladder spanning $10^{-4}$ to $1$.}
    \label{fig:si-optimal-threshold}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption{Correlation between projected Hamiltonian and overlap matrix elements for the UCCSD QSE calculation across all active spaces ($2e2o$ to $8e8o$)}
