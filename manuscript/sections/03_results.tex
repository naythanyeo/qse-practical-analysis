...
In this section, we study the spectral accuracy and representability of low-lying roots generated with QSE using statevector simulations for all active spaces considered using the UCCSD ansatz as ground state references. 
...
The $S_1$ states generally exhibit substantial double-excitation weight, whereas the corresponding $T_1$ and $T_2$ states contain markedly smaller contributions. {\color{red}{You need to quantify using numerical values, basically describe the figure in words using numbers, also mention that the doubles contributions increase with active space size in general, again quantitatively}}
...
While the one-body operator pool provides independent access to singly excited configurations, it does not introduce double-excitations independent of the optimized ground state ansatz. 
The corresponding double-excitation amplitudes therefore cannot be tuned further and remain largely constrained by the VQE reference. 
...
For all active spaces $4e4o$ and larger, $q_1$ overlaps most strongly with $S_2$ rather than $S_1$ \cref{fig:si-q1-best-root-transition-map}. {\color{red}{You need to quantify using numerical values, basically describe the heatmap data in words using numbers}}
...
Examination of the corresponding CASCI wavefunctions shows that $S_1$ carries substantially stronger double-excitation character, whereas $S_2$ is more strongly dominated by single excitations.  {\color{red}{You need to quantify using numerical values, basically describe the heatmap data in words using numbers}}
...
Although UCCSDSinglet imposes additional parameter-tying constraints, both ansatz span the same excitation manifold and yield comparable errors.
...
Taken together with the spectral-error analysis above, these results identify UCCSD as the most cost-effective option among the ansatz considered. 
...
The triplet number operators therefore only contributes when acting on configurations containing singly occupied spatial orbitals. 
...
A second source of near-null excitations observed occurs within the $2e2o$, $2e3o$, and $4e3o$ active spaces. In those cases, the number of nominal QSE basis states exceeds the dimension of the physically accessible subspace within the relevant spin sector. 
...
We term these basis states \textit{stable} because their associated overlap eigenvalues remain consistently of magnitude one or larger. Retaining them therefore does not introduce any strong noise amplifications.
...
However, restricting the QSE subspace to severely degrades the recovered spectrum, producing markedly larger energy errors across the target states (\cref{fig:si-max-stable-error-distribution}). {\color{red}{Please quantify}}
...
This motivates the use of overlap-eigenvalue thresholding to weakly contributing eigenbases.
...
However, a doubles operator pools (even the restricted OV form) scales as $\mathcal{O}\!\left(n_{\mathrm{orb}}^{4}\right)$, compared with $\mathcal{O}\!\left(n_{\mathrm{orb}}^{2}\right)$ for the present one-body pool. 
...
Related approaches have already been explored in quantum error mitigation, where machine-learning, model-based and constrained-based methods exploit systematic structure, correlations or known physical constraints in noisy measurement data to improve estimates of ideal observables \cite{Liao2024MLQEM, Aasen2025ReadoutMitigation, Wang2026RDMErrorPostProcessing}. 
