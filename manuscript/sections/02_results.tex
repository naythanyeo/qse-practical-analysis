...
Here, we examine the full eigenvalue spectrum of S (\cref{fig:eigenvalue-distribution}, upper panel) to understand why these near-null directions arise in the first place. 
...
Each eigenvector of S can then be decomposed into basis states corresponding to each category (\cref{fig:eigenvalue-distribution}, lower panel). 
...
where $n_{\mathrm{orb}}$ is the number of active spatial orbitals (\cref{fig:shot-scaling}, panel (a)). 
Consistent with this trend, the optimal overlap threshold generally increases with the active-space size and decreases with shot count (\cref{fig:shot-scaling}, panel (b)). 
