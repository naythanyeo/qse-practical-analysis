...
def run_molecule_expansion(
    ...
    if molecule_keys.issubset(sv_done[active_space]):
        print(f"{molecule_name} {active_space} {expansion}: all ansatz spin records done, skipping")
        return
    ...
    try:
        ...
        for ansatz_name, ansatz_function in ANSATZ_FUNCTIONS.items():
            ...
            if sv_key in sv_done[active_space]:
                print(f"{molecule_name} {active_space} {ansatz_name} {expansion}: spin SV done, skipping")
                del final_circuit
                gc.collect()
                continue
...
def main():
    ...
    sv_done = {
        active_space: completed_sv_keys(OUTPUT_DIR / f"S2_SV_HS_{active_space}.jsonl")
    ...
    for active_space in ACTIVE_SPACES:
        ...
        if all_keys.issubset(sv_done[active_space]):
            print(f"{active_space}: all spin SV records done, skipping")
            continue
        ...
        sv_file = OUTPUT_DIR / f"S2_SV_HS_{active_space}.jsonl"
